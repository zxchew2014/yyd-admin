...
export const fetchAllTeacherAttendanceClockOut = inputData => async dispatch => {
    ...
    attendancesRef.on(VALUE_KEY, data => {
        ...
        const mapClockInAsc = filterTeacherAttendanceByDate(result, inputData, CAMELCASED_CLOCK_IN);
        const mapTeacherClockInAsc = sortTeacherMapByTeacherName(mapClockInAsc);

        const mapClockOutAsc = filterTeacherAttendanceByDate(result, inputData, CAMELCASED_CLOCK_OUT);
...
export const fetchTeacherAttendanceClockOut = inputData => async dispatch => {
    ...
    attendancesRef.on(VALUE_KEY, data => {
        const result = data.val();
        const mapClockInAsc = filterTeacherAttendanceByDateNBranch(result, inputData, CAMELCASED_CLOCK_IN);

        const mapTeacherClockInAsc = sortTeacherMapByTeacherNameNBranch(mapClockInAsc);

        const mapClockOutAsc = filterTeacherAttendanceByDateNBranch(result, inputData, CAMELCASED_CLOCK_OUT);
        const mapTeacherClockOutAsc = sortTeacherMapByTeacherNameNBranch(mapClockOutAsc);
        const mapFinalAttendance = mergeTeacherAttendance(mapTeacherClockInAsc, mapTeacherClockOutAsc
        );
