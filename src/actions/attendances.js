...
export const fetchTeacherAttendanceClockOut = date => async dispatch => {
    ...
    attendancesRef.on("value", data => {
        ...
        const mapClockOutAsc = filterAttendanceByDate(result, start, end, "clockOut");
        const mapTeacherClockOutAsc = sortTeacherMapByTeacherName(mapClockOutAsc);
        const mapFinalAttendance = mergeTeacherAttendance(mapTeacherClockInAsc, mapTeacherClockOutAsc);
...
const mergeTeacherAttendance = (clockInMap, clockOutMap) => {
    ...
    clockInMap.forEach((value, key) => {
        ...
        Object.keys(clockInArray).forEach(clockInIndex => {
            ...
            if (clockOutArray && clockOutArray.length > 0) {
                const clockInDate = moment(clockInAttendance.clockIn, 'DD/MM/YYYY, HH:mm:ss').format('L');
                Object.keys(clockOutArray).some(clockOutIndex => {
                    const clockOutDate = moment(clockOutArray[clockOutIndex].clockOut, 'DD/MM/YYYY, HH:mm:ss').format('L');
                    const dateCheck = (clockInDate === clockOutDate);
                    const subjectCheck = clockInAttendance.subject === clockOutArray[clockOutIndex].subject;
                    const branchCheck = clockInAttendance.branch === clockOutArray[clockOutIndex].branch;
                    const primaryCheck = comparePrimaryClass(clockInAttendance.primary, clockOutArray[clockOutIndex].primary)
    ...
    clockOutMap.forEach((value, key) => {
        ...
        Object.keys(clockOutArray).forEach(clockOutIndex => {
            ...
            if (clockInArray && clockInArray.length > 0) {
                const clockOutDate = moment(clockOutAttendance.clockOut, 'DD/MM/YYYY, HH:mm:ss').format('L');
                Object.keys(clockInArray).some(clockInIndex => {
                    const clockInDate = moment(clockInArray[clockInIndex].clockIn, 'DD/MM/YYYY, HH:mm:ss').format('L');
                    const dateCheck = (clockInDate === clockOutDate);
                    const subjectCheck = clockOutAttendance.subject === clockInArray[clockInIndex].subject;
                    const branchCheck = clockOutAttendance.branch === clockInArray[clockInIndex].branch;
                    const primaryCheck = comparePrimaryClass(clockOutAttendance.primary, clockInArray[clockInIndex].primary)
    ...
    clockInMap.forEach((value, key) => {
        value.sort((a, b) =>
            moment(a.timestamp, 'DD/MM/YYYY, HH:mm:ss').toDate() - moment(b.timestamp, 'DD/MM/YYYY, HH:mm:ss').toDate()
        )
    })
