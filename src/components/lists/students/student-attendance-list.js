...
class StudentAttendanceList extends React.Component {
    render() {
        const {startDate, endDate, branch, batch, attendanceStudents} = this.props;
        ...
        if (attendanceStudents.length > 0) {
            return [
                <GenerateStudentAttendanceList startDate={startDate} endDate={endDate} branch={branch} batch={batch}/>,
                <hr/>
            ];
        }
...
export default connect(mapStateToProps, {})(StudentAttendanceList);